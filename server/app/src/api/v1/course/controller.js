...
exports.publishCourse = async (req, res, next) => {
  ...
  try {
    ...
    // Check if course has required fields before publishing
    if (!course.title || !course.categoryId || !course.subCategoryId) {
    ...
    await createActivityLog({
      ...
      desc: `${course._id} course id is published`,
      ip,
      user: _id,
    });
    ...
    res.status(200).json({
      code: 200,
      status: "success",
      message: req.__("coursePublishSucc") || "Course published successfully",
      data: { course },
    });
  } catch (error) {
    next(error, req, res);
